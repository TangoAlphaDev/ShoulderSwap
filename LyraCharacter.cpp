...
void ALyraCharacter::GetLifetimeReplicatedProps(TArray< FLifetimeProperty >& OutLifetimeProps) const
{
	...
	// @TangoAlphaDev
	// Setting replicating for variable
	DOREPLIFETIME_CONDITION(ALyraCharacter, isDriver, COND_None);
	DOREPLIFETIME_CONDITION(ALyraCharacter, isChutePulled, COND_None);
	// @TangoAlphaDev
...
void ALyraCharacter::Landed(const FHitResult& Hit)
{
	...
	{
		// Call the ApplyFallDamage function with the velocity and multiplier
		HealthComponent->ApplyFallDamage(Velocity, VelocityLimit, FallDamageMultiplier);
	} 
}
// @TangoAlphaDev
...
// @TangoAlphaDev
void ALyraCharacter::UninitializeAbilitySystemComponent()
{
	// Uninitialize the ASC if we're still the avatar actor (otherwise another pawn already did it when they became the avatar actor)
	if (const ULyraAbilitySystemComponent* LyraASC = GetLyraAbilitySystemComponent())
	{
...
// @TangoAlphaDev
void ALyraCharacter::SetLeftCameraMode(bool UseLeftMode)
{
	isLeftCamera = UseLeftMode;
}

bool ALyraCharacter::GetLeftCameraMode() const
{
	return isLeftCamera;
}
// @TangoAlphaDev